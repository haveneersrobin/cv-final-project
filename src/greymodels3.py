...
class GreyLevelModel(object):
    ...
    def build(self, images, gimages, models, point_ind, k):
        ...
        for ind in range(len(images)):
            self.profiles.append(Profile(images[ind], gimages[ind], models[ind].get_list().tolist(), point_ind, k))
...
class Profile(object):
    ...
    def __init__(self, image, grad_image, model, point_ind, k):
        ...
        self.model_point = model[point_ind, :]
        self.k = k
        self.normal = self.__calculate_normal(model[(point_ind-1) % 40, :],
                                              model[(point_ind+1) % 40, :])
    ...
    def __calculate_normal(self, p_prev, p_next):
        ...
        """
        n1 = np.array([p_prev[1] - self.model_point[1], self.model_point[0] - p_prev[0]])
        # n1 = utils.normal(p_prev, self.model_point)
        n2 = np.array([self.model_point[1] - p_next[1], p_next[0] - self.model_point[0]])
