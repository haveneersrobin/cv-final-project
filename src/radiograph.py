...
def main():
    ...
    img = cv2.imread(paths.RADIO+'denoised.png')
    cv2.imwrite('../report/img/'+'Doriginal.png', img)
    gray_img = to_grayscale(img)
    cv2.imwrite('../report/img/'+'Dgrayscale.png', gray_img)    
    median = img = medianfilter(gray_img,9)
    cv2.imwrite('../report/img/'+'Dmedian.png', median)    
    bilateral = cv2.bilateralFilter(median, 9, 150, 150)
    cv2.imwrite('../report/img/'+'Dbilateral.png', bilateral)    
    whiteHat = whitehat(bilateral,350)
    cv2.imwrite('../report/img/'+'Dwhitehat.png', whiteHat)
    blackHat = blackhat(bilateral,80)
    cv2.imwrite('../report/img/'+'Dblackhat.png', blackHat)
    hatted = hatfilter(bilateral, whiteHat, blackHat) 
    cv2.imwrite('../report/img/'+'Dhatted.png', hatted)    
    ahe = AHE(img, 0.8, 256)
    cv2.imwrite('../report/img/'+'Dahe.png', ahe)    
    sobelx = cv2.Sobel(hatted, cv2.CV_64F, 1, 0, ksize=3)
    sobely = cv2.Sobel(hatted, cv2.CV_64F, 0, 1, ksize=3)
    abs_grad_x = cv2.convertScaleAbs(sobelx)
    abs_grad_y = cv2.convertScaleAbs(sobely)
    sobel = cv2.addWeighted(abs_grad_x, 0.5, abs_grad_y, 0.5, 0)
    cv2.imwrite('../report/img/'+'Dsobel.png', sobel)
